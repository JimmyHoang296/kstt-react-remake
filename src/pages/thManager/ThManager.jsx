          ...
          {rows.map((r) => (
            <tr key={r.id} className="hover:bg-gray-50">
              ...
              <Td className="max-w-xs">
                <p className="line-clamp-2 text-gray-600">{r.violation_text}</p>
              </Td>
              <Td className="text-right whitespace-nowrap">{r.loss_value ? r.loss_value + ' đ' : ''}</Td>
              <Td className="text-right whitespace-nowrap">{r.recover_value ? r.recover_value + ' đ' : ''}</Td>
              <Td>
                {r.Note && (
                  <span className={`text-xs px-2 py-0.5 rounded-full ${BADGE[r.Note] || 'bg-gray-100 text-gray-600'}`}>
                    {r.Note}
                  </span>
                )}
              </Td>
          ...
          {rows.map((r) => (
            <tr key={r.id} className="hover:bg-gray-50">
              ...
              <Td className="max-w-[160px]">
                <p className="line-clamp-2">{r.disciplinary_action}</p>
              </Td>
              <Td>
                {r.status && (
                  <span className={`text-xs px-2 py-0.5 rounded-full ${BADGE[r.status] || 'bg-gray-100 text-gray-600'}`}>
                    {r.status}
                  </span>
                )}
              </Td>
...
const ThManager = () => {
  ...
  useEffect(() => {
    const params = { role, userName, emps };
    setLoading(true);
    setError('');
    Promise.all([
      ...
      if (r1.success) setRows1(r1.data);
      else setError(r1.message);
      if (rk.success) setRowsKhac(rk.data);
      else setError((e) => e || rk.message);
